"use client";

import * as React from "react";

interface MessageSquareIconProps extends React.SVGProps<SVGSVGElement> {}

export const MessageSquareIcon = React.forwardRef<SVGSVGElement, MessageSquareIconProps>(
  ({ className = "", ...props }, ref) => {
    return (
      <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className={className}
        ref={ref}
        {...props}
      >
        <path
          d="M14 10C14 10.3536 13.8595 10.6928 13.6095 10.9428C13.3594 11.1929 13.0203 11.3333 12.6667 11.3333H4.66667L2 14V3.33333C2 2.97971 2.14048 2.64057 2.39052 2.39052C2.64057 2.14048 2.97971 2 3.33333 2H12.6667C13.0203 2 13.3594 2.14048 13.6095 2.39052C13.8595 2.64057 14 2.97971 14 3.33333V10Z"
          fill="currentColor"
        />
      </svg>
    );
  },
);

MessageSquareIcon.displayName = "MessageSquareIcon";
