...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!medspaId) {
      return NextResponse.json({ error: "Missing required field: medspaId" }, { status: 400 });
    }
    ...
    if (!keragonRes.ok) {
      return NextResponse.json({ error: data }, { status: keragonRes.status });
    }

    return NextResponse.json({ message: "Workflow triggered", run: data }, { status: 200 });
  } catch (error) {
